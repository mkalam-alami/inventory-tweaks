...
public class InvTweaksHandlerShortcuts extends InvTweaksObfuscation {
        ...
        synchronized(this) {
            if (shortcut.type == InvTweaksShortcutType.MOVE_TO_SPECIFIC_HOTBAR_SLOT) {
                ...
                if (toIndex != -1) {
                    ...
                    case MOVE_ONE_STACK:
                    {
                        yu slot = container.getSlot(shortcut.fromSection, shortcut.fromIndex);
    ...
    private void moveAll(ShortcutConfig shortcut, aan stackToMatch) throws TimeoutException {
        int toIndex = getNextTargetIndex(shortcut), newIndex;
        boolean success;
        for (yu slot : container.getSlots(shortcut.fromSection)) {
    ...
    private int getNextTargetIndex(ShortcutConfig shortcut) {
        ...
        if (!shortcut.forceEmptySlot) {
            int i = 0;
            for (yu slot : container.getSlots(shortcut.toSection)) {
                if (hasStack(slot)) {
