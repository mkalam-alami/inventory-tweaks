...
public class InvTweaksHandlerSorting extends InvTweaksObfuscation {
    ...
    public InvTweaksHandlerSorting(Minecraft mc, InvTweaksConfig config,
        ...
        for (int i = 0; i < size; i++) {
            ...
            qs stack = containerMgr.getItemStack(i);
            if (stack != null) {
        ...
        if (algorithm != ALGORITHM_DEFAULT) {
            ...
            if (algorithm == ALGORITHM_INVENTORY) {
                ...
                if (globalContainer.hasSection(InvTweaksContainerSection.CRAFTING_IN)) {
                    List<pa> craftingSlots = globalContainer.getSlots(InvTweaksContainerSection.CRAFTING_IN);
                    int emptyIndex = globalContainer.getFirstEmptyIndex(InvTweaksContainerSection.INVENTORY);
                    if (emptyIndex != -1) {
                        for (pa craftingSlot : craftingSlots) {
                ...
                for (int i = size - 1; i >= 0; i--) {
                    qs from = containerMgr.getItemStack(i);
                    if (from != null) {

                        // Move armor parts
	                    qq fromItem = getItem(from);
	                    if (isDamageable(fromItem)) {
	                        if (sortArmorParts) {
	                             if (isItemArmor(fromItem)) {
	                            	 pc fromItemArmor = asItemArmor(fromItem);
	                                 if (globalContainer.hasSection(InvTweaksContainerSection.ARMOR)) {
	                                     List<pa> armorSlots = globalContainer.getSlots(InvTweaksContainerSection.ARMOR);
	                                     for (pa slot : armorSlots) {
	                                    	boolean move = false;
	                                    	if (!hasStack(slot)) {
	                                    		move = true;
	                                    	}
                        ...
                        // Stackable objects are never damageable
                            ...
                            for (Integer lockPriority : lockPriorities) {
                                if (lockPriority > 0) {
                                    qs to = containerMgr.getItemStack(j);
        ...
        if (lockPriorities[i] <= priority) {
            ...
            // Try to swap/merge
                ...
                if (canBeSwappedOrMerged) {
                    ...
                    qs remains = containerMgr.getItemStack(i);
    ...
    private boolean isOrderedBefore(int i, int j) {
        ...
        qs iStack = containerMgr.getItemStack(i),
    ...
    private int getItemOrder(qs item) {
