...
public class InvTweaksHandlerSorting extends InvTweaksObfuscation {
        ...
        if (algorithm != ALGORITHM_DEFAULT) {
            ...
            if (algorithm == ALGORITHM_EVEN_STACKS) {
                ...
                if (globalContainer.hasSection(InvTweaksContainerSection.CRAFTING_IN)) {
                    List<sr> craftingSlots = globalContainer.getSlots(InvTweaksContainerSection.CRAFTING_IN);
                    int emptyIndex = globalContainer.getFirstEmptyIndex(InvTweaksContainerSection.INVENTORY);
                    if (emptyIndex != -1) {
                        for (sr craftingSlot : craftingSlots) {
                ...
                for (int i = size - 1; i >= 0; i--) {
                	ur from = containerMgr.getItemStack(i);
                    if (from != null) {

                        // Move armor parts
                    	up fromItem = getItem(from);
	                    if (isDamageable(fromItem)) {
	                        if (sortArmorParts) {
	                             if (isItemArmor(fromItem)) {
	                            	 su fromItemArmor = asItemArmor(fromItem);
	                                 if (globalContainer.hasSection(InvTweaksContainerSection.ARMOR)) {
	                                     List<sr> armorSlots = globalContainer.getSlots(InvTweaksContainerSection.ARMOR);
	                                     for (sr slot : armorSlots) {
	                                    	boolean move = false;
	                                    	if (!hasStack(slot)) {
	                                    		move = true;
	                                    	}
                        ...
                        // Stackable objects are never damageable
                            ...
                            for (Integer lockPriority : lockPriorities) {
                                if (lockPriority > 0) {
                                	ur to = containerMgr.getItemStack(j);
    ...
    private int move(int i, int j, int priority) throws TimeoutException {
    	...
    	ur from = containerMgr.getItemStack(i), to = containerMgr.getItemStack(j);
        ...
        if (lockPriorities[i] <= priority) {
            ...
            // Try to swap/merge
                ...
                if (canBeSwappedOrMerged) {
                    ...
                    ur remains = containerMgr.getItemStack(i);
    ...
    private boolean isOrderedBefore(int i, int j) {
    	...
    	ur iStack = containerMgr.getItemStack(i), jStack = containerMgr.getItemStack(j);
    ...
    private int getItemOrder(ur itemStack) {
