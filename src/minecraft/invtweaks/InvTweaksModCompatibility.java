...
public class InvTweaksModCompatibility {
    ...
    public boolean isSpecialChest(GuiScreen guiScreen, Container container) {
        return isExact(guiScreen, "cpw.mods.ironchest.client.GUIChest") // Iron chests (formerly IC2)
    ...
    public int getSpecialChestRowSize(GuiContainer guiContainer, Container container, int defaultValue) {
        if(isExact(guiContainer, "cpw.mods.ironchest.client.GUIChest")) { // Iron chests (formerly IC2)
            try {
    ...
    public boolean isSpecialInventory(GuiScreen guiScreen) {
        if(isExact(guiScreen, "micdoodle8.mods.galacticraft.core.client.gui.GCCoreGuiTankRefill")) {
            return true;
        }
        try {
